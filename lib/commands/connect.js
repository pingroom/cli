...
function printProvisionedRobot(agent) {
...
/**
 * Keep only an absolute HTTP(S) latest-pings URL. Pairing data is controlled by
 * the selected API host and is printed to a terminal, so malformed schemes and
 * control characters are discarded instead of being persisted indefinitely.
 *
...
async function connectByPairing(
  apiBase,
  ask,
  { qr = true, json: jsonOut = false, agentLabel = DEFAULT_AGENT_LABEL } = {},
) {
  for (;;) {
    ...
    if (!jsonOut) printProvisionedRobot(metadata.agent);
 ...
 */
async function connectByEmail(apiBase, ask, agentLabel) {
  const registration = await registerAnonymous(apiBase, agentLabel);
  const headers = { Authorization: `Bearer ${registration.credential}` };
  printProvisionedRobot(registration.agent);
...
export async function connect(args) {
  ...
  try {
    ...
    const cred = choice === '2'
      ? await connectByEmail(apiBase, ask, agentLabel)
      : await connectByPairing(apiBase, ask, { agentLabel });
...
export async function reconnect(args) {
  ...
  let cred;
  try {
    cred = await connectByPairing(apiBase, ask, { agentLabel });
  } finally {
    prompter.close();
  }
