...
import { EXIT, INSTALL_APP_URL } from '../constants.js';
import {
...
// id: the phone shows it verbatim ("PingRoom CLI wants to connect"). Callers
// may make the robot more specific with --agent-label (for example, "OpenClaw
// on studio-mac") without changing the default existing installs send.
const DEFAULT_AGENT_LABEL = 'PingRoom CLI';
...
/**
 * Prefer the explicit QR field while retaining old-server compatibility.
 */
...
/** Browser approval uses its own link without changing the legacy QR URL. */
export function pairingBrowserUrl(pairing) {
  const candidate = typeof pairing?.pair_browser_url === 'string'
    ? stripControlChars(pairing.pair_browser_url)
    : '';
  return candidate.trim() === '' ? stripControlChars(pairing?.pair_url ?? '') : candidate;
}
...
function requestedAgentLabel(value) {
  if (value === undefined) return DEFAULT_AGENT_LABEL;
  const label = optionalTerminalText(value);
  if (!label || label !== String(value).trim()) {
    fail('--agent-label must be non-empty text without control characters', EXIT.USAGE);
  }
  if ([...label].length > 100) {
    fail(`--agent-label must be at most 100 characters (got ${[...label].length})`, EXIT.USAGE);
  }
  return label;
}

function normalizeAgentProfile(value, fallbackLabel) {
  const agent = isJsonObject(value) ? value : {};
  const profile = isJsonObject(agent.profile) ? agent.profile : {};
  const displayName = optionalTerminalText(profile.display_name)
    || optionalTerminalText(agent.label)
    || fallbackLabel;
  const handle = optionalTerminalText(profile.handle) || optionalTerminalText(agent.handle);
  const avatarId = optionalTerminalText(profile.avatar_id);
  const avatarUrl = validHttpUrl(profile.avatar_url) || null;

  return {
    ...(optionalTerminalText(agent.id) ? { id: optionalTerminalText(agent.id) } : {}),
    label: optionalTerminalText(agent.label) || displayName,
    handle,
    profile: {
...
function normalizeOwner(value, fallbackAccount) {
  const owner = isJsonObject(value) ? value : {};
  const account = isJsonObject(fallbackAccount) ? fallbackAccount : {};
  const name = optionalTerminalText(owner.name) || optionalTerminalText(account.name);
  return name ? { name } : null;
}

function normalizeRoom(value) {
  if (!isJsonObject(value)) return null;
  const inviteCode = optionalTerminalText(value.invite_code);
  if (!inviteCode) return null;
  return {
    ...(optionalTerminalText(value.id) ? { id: optionalTerminalText(value.id) } : {}),
    invite_code: inviteCode,
    name: optionalTerminalText(value.name),
  };
}

function normalizeRoomMembership(value) {
  if (!isJsonObject(value)) return null;
  const status = value.status === 'active' || value.status === 'removed' ? value.status : null;
  const room = normalizeRoom(value.room);
  if (!status && !room) return null;
  return {
    status,
    joined_at: isNullableString(value.joined_at) ? value.joined_at : null,
    removed_at: isNullableString(value.removed_at) ? value.removed_at : null,
    room,
  };
}

/** Select and sanitize the additive v2 identity fields safe to print in logs. */
function pairingMetadata(value, { agent, account, room } = {}) {
  const source = isJsonObject(value) ? value : {};
  const normalizedAgent = normalizeAgentProfile(source.agent ?? agent, agent?.profile?.display_name ?? DEFAULT_AGENT_LABEL);
  const owner = normalizeOwner(source.owner, source.account ?? account);
  const homeRoom = normalizeRoom(source.home_room) || normalizeRoom(room);
  const hasMembership = Object.hasOwn(source, 'room_membership');
  const membership = source.room_membership === null
    ? null
    : normalizeRoomMembership(source.room_membership);

  return {
    ...(source.flow_version === 2 ? { flow_version: 2 } : {}),
    ...(source.claim_mode === 'agent_identity' ? { claim_mode: 'agent_identity' } : {}),
    agent: normalizedAgent,
    ...(owner ? { owner } : {}),
    ...(homeRoom ? { home_room: homeRoom } : {}),
    ...(hasMembership ? { room_membership: membership } : {}),
  };
}

function agentDisplay(agent, fallbackHandle) {
  const name = optionalTerminalText(agent?.profile?.display_name)
    || optionalTerminalText(agent?.label)
    || 'Robot';
  const handle = optionalTerminalText(agent?.profile?.handle)
    || optionalTerminalText(agent?.handle)
    || optionalTerminalText(fallbackHandle);
  return `${name}${handle ? ` (@${handle.replace(/^@/, '')})` : ''}`;
}

function printMobileSetup({ pendingClaim = false } = {}) {
  process.stdout.write(`  Install or open PingRoom on your phone and sign in: ${INSTALL_APP_URL}\n`);
  process.stdout.write('  It receives urgent Pings, questions, approvals, handoffs, and live progress.\n');
  process.stdout.write('  Installing the app does not claim a robot or grant it access.\n');
  if (pendingClaim) {
    process.stdout.write('  Keep this pairing running; after installing, return to the same claim link before it expires.\n');
  }
}

function printProvisionedRobot(agent, { showMobileSetup = true } = {}) {
  process.stdout.write(`  Created robot: ${agentDisplay(agent)}\n`);
  if (showMobileSetup) printMobileSetup({ pendingClaim: true });
  process.stdout.write('  Claim this robot in PingRoom, then choose its home room and room access.\n');
}

/** POST /api/agent/auth — anonymous registration, yields a robot + pre-claim credential. */
async function registerAnonymous(apiBase, agentLabel) {
  const { res, json } = await httpJson('POST', `${apiBase}/api/agent/auth`, {
    // Pairing grants are server-owned. Sending a client-maintained scope list
    // made older installs reconnect every time that list changed.
    body: { type: 'anonymous', agent_label: agentLabel },
  });
  ...
  return {
    ...
    agent: normalizeAgentProfile(json.agent, agentLabel),
  };
}

/**
 * Durable status for an already-connected robot. Keep the home room separate
 * from reach: an all-room grant still has one delivery/home room.
 */
function connectedLine(cred) {
  const handle = optionalTerminalText(cred.handle);
  const who = handle ? `@${handle.replace(/^@/, '')}` : 'this robot';
  const room = optionalTerminalText(cred.room?.name)
    || optionalTerminalText(cred.room?.invite_code);
  const access = cred.room_access ?? cred.roomAccess;

  if (access === 'all') {
    return `✓ Robot ${who} is connected${room ? ` · home #${room}` : ' · no home room'} · access all rooms`;
  }

  if (!room) return `✓ Robot ${who} is connected · no home room`;

  const extra = Math.max(0, (Array.isArray(cred.rooms) ? cred.rooms.length : 0) - 1);
  return `✓ Robot ${who} is connected · home #${room}${extra > 0 ? ` · access +${extra} more` : ''}`;
}

/** Human-readable connection receipt, including the server's reusable read URL. */
function connectedOutput(cred) {
  const latestPings = pairingLinks(
    cred?.links,
    cred?.apiBase ?? cred?.api_url,
  )?.latest_pings;
  return `${connectedLine(cred)}${latestPings ? `\n  Latest pings API: ${latestPings}` : ''}`;
}

/** Human receipt for the claim that just completed, with one useful next step. */
function claimedOutput(cred) {
  const room = optionalTerminalText(cred.room?.name)
    || optionalTerminalText(cred.room?.invite_code);
  const owner = optionalTerminalText(cred.owner?.name) || optionalTerminalText(cred.account?.name);
  const access = cred.room_access ?? cred.roomAccess;
  const roomCount = Array.isArray(cred.rooms) ? cred.rooms.length : 0;
  let output = `✓ ${agentDisplay(cred.agent, cred.handle)} was claimed${owner ? ` by ${owner}` : ''}`;
  output += room ? ` and joined #${room}.` : '. No home room is set yet.';

  if (access === 'all') output += '\n  Room access: all rooms';
  else if (access === 'selected') {
    const selectedCount = roomCount || (room ? 1 : 0);
    output += `\n  Room access: ${selectedCount} selected room${selectedCount === 1 ? '' : 's'}`;
  }

  output += room
    ? '\n  Try it: pingroom ping -m "Hello from this robot"'
    : '\n  Next: choose a home room in PingRoom → Connected Agents.';
  return output;
}

/**
 * Keep only safe read/install links. Pairing data is controlled by the selected
 * API host and is printed to a terminal, so malformed schemes and control
 * characters are discarded instead of being persisted indefinitely.
 *
 * Older servers do not return `links`. Their stable notifications endpoint is a
 * useful, token-free fallback: callers still authenticate in the usual header.
 */
export function pairingLinks(value, apiBase) {
  const source = value && typeof value === 'object' && !Array.isArray(value) ? value : {};
  const supplied = validHttpUrl(source.latest_pings);
  let latestPings = supplied;

  if (!latestPings) {
    try {
      const parsedBase = new URL(apiBase);
      const normalizedBase = `${parsedBase.origin}${parsedBase.pathname}`.replace(/\/+$/, '');
      latestPings = validHttpUrl(
        `${normalizedBase}/api/agent/notifications?limit=25&page=1`,
      );
    } catch {
      // The install handoff below is still useful even if an invalid API base
      // prevents construction of the old-server latest-pings fallback.
    }
  }

  return {
    ...(latestPings ? { latest_pings: latestPings } : {}),
    install_app: pairingInstallUrl(source),
  };
}

/**
 * Select the server's additive install link only when it is the canonical,
 * token-free mobile handoff. A configured API host controls pairing JSON, so
 * arbitrary URLs or query parameters must never become an agent's install CTA.
 */
export function pairingInstallUrl(value) {
  const source = value && typeof value === 'object' && !Array.isArray(value) ? value : {};
  const links = source.links && typeof source.links === 'object' && !Array.isArray(source.links)
    ? source.links
    : {};
  const mobileApp = source.mobile_app && typeof source.mobile_app === 'object'
    && !Array.isArray(source.mobile_app)
    ? source.mobile_app
    : {};
  const candidates = [
    source.app_install_url,
    source.install_app,
    source.install_url,
    links.install_app,
    mobileApp.install_url,
  ];

  for (const candidate of candidates) {
    const supplied = validHttpUrl(candidate);
    if (supplied === INSTALL_APP_URL) return supplied;
  }
...
function validHttpUrl(value) {
  ...
  const clean = stripControlChars(value).trim();
  if (clean !== value.trim()) return undefined;
  try {
...
function activationFailureCode(result) {
  const direct = optionalTerminalText(result?.json?.code);
  if (direct) return direct;
  return optionalTerminalText(result?.json?.error?.code);
}
...
async function activateInboxAfterPairing(cred) {
  ...
  if (!ensured.res?.ok) {
    ...
    const instruction = activationFailureCode(ensured) === 'recipient_not_ready'
      ? `Install or update PingRoom: ${INSTALL_APP_URL}\n`
        + '  Open it, sign in, and enable notifications; then run "pingroom activate" again.'
      : undefined;
    activationIncomplete(detail, instruction);
    return false;
  }
...
/** Retry activation only for the durable robot credential saved by this CLI. */
export async function activateStoredInbox(args) {
  ...
  if (args.token !== undefined) {
    fail('pingroom activate uses the saved robot credential; remove --token', EXIT.USAGE);
  }
  ...
  if (!credential) {
    fail('no saved robot credential; run "pingroom" in an interactive terminal first', EXIT.USAGE);
  }
  if (!credential.room || !isNonEmptyString(credential.room.invite_code)) {
    // An all-rooms grant normally carries an eligible private home room,
    // but older servers (or accounts with none) may still return no destination.
    // The fix is picking one, not repeatedly pairing again.
    fail(
      credential.room_access === 'all'
        ? 'this robot was granted all rooms but has no home room; pick one in the PingRoom app under Connected Agents, then run "pingroom activate" again'
        : 'the saved robot has no home room; choose one under Connected Agents before running "pingroom activate"',
      EXIT.USAGE,
    );
  }
  ...
  if (!isNonEmptyString(credential.api_url)) {
    fail('the saved robot credential has no trusted API origin; pair again before running "pingroom activate"', EXIT.USAGE);
  }
  ...
  try {
    ...
    fail('the saved robot credential has an invalid API origin; pair again', EXIT.USAGE);
  }
  ...
  process.stdout.write(`${connectedOutput(credential)}\n`);
 ...
 */
async function connectByPairing(
  apiBase,
  ask,
  {
    qr = true,
  ...
  for (;;) {
    const registration = await registerAnonymous(apiBase, agentLabel);
    const headers = { Authorization: `Bearer ${registration.credential}` };
    ...
    const pairUrl = pairingBrowserUrl(start.json);
    const qrUrl = pairingQrUrl(start.json);
    const installUrl = pairingInstallUrl(start.json);
    const metadata = pairingMetadata(start.json, { agent: registration.agent });
    ...
    if (!jsonOut) printProvisionedRobot(metadata.agent, { showMobileSetup });
    ...
    if (jsonOut) {
      writeEvent({
        ...
        links: { install_app: installUrl },
        ...metadata,
      });
    } else {
      process.stdout.write(`${drew ? '  Or open' : '  Open'}: ${pairUrl}\n`);
      process.stdout.write('  Waiting for claim… ');
    }
    ...
    while (Date.now() < deadline) {
      ...
      if (status === 'active') {
        ...
        const activeMetadata = pairingMetadata(json, {
        ...
        const cred = {
          ...
          handle: json.handle ?? activeMetadata.agent?.handle,
          room: json.room ?? activeMetadata.home_room ?? null,
          rooms: Array.isArray(json.rooms) ? json.rooms : [],
          roomAccess: typeof json.room_access === 'string' ? json.room_access : null,
          account: json.account,
          links: pairingLinks(json.links, apiBase),
          apiBase,
          ...activeMetadata,
        };
        saveCredential(cred);
        if (jsonOut) {
          ...
          writeEvent({
            ...
            rooms: cred.rooms,
            ...(cred.links ? { links: cred.links } : {}),
            api_url: apiBase,
            ...activeMetadata,
          });
        } else {
          process.stdout.write(`${claimedOutput(cred)}\n`);
        }
...
async function connectByEmail(
  ...
  const registration = await registerAnonymous(apiBase, agentLabel);
  const headers = { Authorization: `Bearer ${registration.credential}` };
  printProvisionedRobot(registration.agent, { showMobileSetup });
  ...
  for (let attempt = 1; attempt <= 3; attempt++) {
    ...
    if (done.res.ok && done.json && typeof done.json.credential === 'string') {
      const activeMetadata = pairingMetadata(done.json, {
      ...
      const cred = {
        ...
        handle: done.json.handle ?? activeMetadata.agent?.handle,
        room: done.json.room ?? activeMetadata.home_room ?? null,
        rooms: Array.isArray(done.json.rooms) ? done.json.rooms : [],
        roomAccess: typeof done.json.room_access === 'string' ? done.json.room_access : null,
        account: done.json.account,
        links: pairingLinks(done.json.links, apiBase),
        apiBase,
        ...activeMetadata,
      };
      saveCredential(cred);
...
export async function connect(args) {
  ...
  const apiBase = resolveApiBase(args);
  requireSafeUrl('--api', apiBase);
  const agentLabel = requestedAgentLabel(args.agent_label);

  const prompter = createPrompter();
  const ask = (question) => prompter.ask(question);
  try {
    process.stdout.write('  Not connected. PingRoom will create a separate robot for this tool.\n');
    printMobileSetup();
    process.stdout.write('    1) Claim the robot with a QR code\n');
    process.stdout.write('    2) Email me a code to claim the robot\n');
    ...
    const cred = choice === '2'
      ? await connectByEmail(apiBase, ask, agentLabel, { showMobileSetup: false })
      : await connectByPairing(apiBase, ask, { agentLabel, showMobileSetup: false });
...
export async function bare(args) {
  ...
  if (stored) {
    process.stdout.write(`${connectedOutput(stored)}\n`);
    const room = resolveRoom(args);
    if (room) process.stdout.write(`Default room: ${room}\n`);
    process.stdout.write(`\n${HELP}\n`);
    return EXIT.OK;
...
/**
 * Replace an existing connection with a freshly approved one. Ordered so that
 * any failure is a no-op:
 *
...
export async function reconnect(args) {
  ...
  if (!isInteractive()) {
    fail('reconnect needs an interactive terminal to show the QR code. Run "pingroom pair" instead — it prints the claim link and waits.', EXIT.USAGE);
  }
  ...
  let cred;
  try {
    cred = await connectByPairing(apiBase, ask, { agentLabel, showMobileSetup: false });
  } finally {
    prompter.close();
  }
...
function printReplaceNotice() {
  process.stdout.write('  Reconnecting creates a replacement robot for this CLI.\n');
  process.stdout.write('  Your current connection keeps working until the replacement is claimed,\n');
  process.stdout.write('  then its credential is revoked — any other machine or CI job using that same\n');
...
/**
 * Pairing for machines with no terminal: print the claim link, wait once,
...
export async function pair(args) {
  ...
  if (args._ && args._.length > 0) {
    fail('usage: pingroom pair [--api <url>] [--agent-label <name>] [--json]', EXIT.USAGE);
  }
  ...
  const cred = await connectByPairing(apiBase, null, {
    qr: false,
    json: jsonOut,
    agentLabel,
  });
