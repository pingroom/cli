...
async function registerAnonymous(apiBase) {
  const { res, json } = await httpJson('POST', `${apiBase}/api/agent/auth`, {
    ...
    body: { type: 'anonymous', agent_label: AGENT_LABEL },
  });
    ...
    while (Date.now() < deadline) {
      ...
      if (status === 'active') {
        ...
        if (jsonOut) {
          ...
          process.stdout.write(`${connectedOutput(cred)}\n`);
        }
...
async function connectByEmail(apiBase, ask) {
  ...
  for (let attempt = 1; attempt <= 3; attempt++) {
    ...
    if (done.res.ok && done.json && typeof done.json.credential === 'string') {
      ...
      process.stdout.write(`${connectedOutput(cred)}\n`);
      if (!cred.room) {
        process.stdout.write('  No private delivery room is available yet. Pick one under Connected Agents.\n');
      }
      return cred;
    }
...
function printReplaceNotice() {
  process.stdout.write('  Reconnecting replaces this CLI\'s saved connection.\n');
