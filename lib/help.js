...
import { BUILTIN_API, DEFAULT_API, INSTALL_APP_URL } from './constants.js';
...
export const HELP_MCP = `mcp:
  ...
  personal PingRoom profile. If the person does not have PingRoom, send them
  ${INSTALL_APP_URL} and ask them to install or open it and sign in before
  pairing. Installation is not consent.`;
...
export const HELP_ACTIVATE = `activate:
  pingroom activate                Send one test Question to your phone to prove the
                                   ...
                                   On recipient_not_ready, keep the saved
                                   connection and follow the server's explanation.
                                   Install or update PingRoom from ${INSTALL_APP_URL},
                                   open it, sign in, enable notifications, then
                                   run this command again.`;

export const HELP_REDEEM = `redeem (agent token required; consent scope pingroom:codes:redeem):
  pingroom redeem <code>           Apply a gifted or promotional Pro code to the
                                   human account that connected this agent.
      --code <code>     Alternative to the positional code
      --json            Print the redemption receipt as JSON

  Codes contain 12 letters or digits; surrounding spaces and letter case are
  normalized. With no code argument, reads PINGROOM_REDEEM_CODE, which keeps
  the code out of command arguments. No room or existing Pro plan is required.
  Full-access connections include redemption; legacy partial connections may
  need "pingroom reconnect". This consumes the code and never auto-retries.`;
...
export const HELP_TAIL = `Connecting:
  Before pairing, have the person claiming the robot install or open PingRoom
  on their phone and sign in:
    ${INSTALL_APP_URL}
  That is where urgent Pings, questions, approvals, handoffs, and live progress
  arrive. Installing the app does not claim a robot or grant it access.
  ...
  Either way the robot gets a home room, so Inbox and Handoff delivery works.
  "config set default_room" enables room-addressed commands.
  ...
  terminal (it prints the robot's claim link and waits). If a pairing process
  is already waiting, keep it running and reuse its claim link. After installing
  the app, return to that link before it expires.
...
export const HELP = [
  ...
  HELP_LISTEN, HELP_LIVE, HELP_HOOK, HELP_MCP, HELP_SKILLS, HELP_REDEEM, HELP_ACTIVATE, HELP_CONFIG,
...
export const COMMAND_HELP_SECTIONS = {
  ...
  pingroom rooms join <code> [--password <password>]
  ...
  pingroom actions set-all --room <code> --actions -        (array on stdin)
                                    ...
                                    fields as "actions set" plus action_number:
                                      {"action_number":1,"label":"Deployed",
                                       "icon":"✅","sound":"ting"}
  pingroom actions trigger <1-4> --room <code> [--require-ack] [--urgent]
                                    Modifiers apply to this press only.
  ...
  pingroom pair                     Create a robot, print its PingRoom claim link,
                                    ...
                                    Before pairing, ask the human to install or
                                    open PingRoom and sign in at ${INSTALL_APP_URL}.
                                    Installation does not claim the robot or grant
                                    it access. If a pairing process is still waiting,
                                    keep it running and reuse its claim link until
                                    it expires; do not start a replacement.
      ...
      --json                 One JSON object per line on stdout:
                             {"event":"pair_url","pair_url":…,"agent":…,
                              "links":{"install_app":"${INSTALL_APP_URL}"}}
                             first, then {"event":"connected","handle":…,
                             "links":{"latest_pings":…,"install_app":…}}.
